...
def load_base_move_actor(path: Path) -> Dict[str, torch.Tensor]:
    obj = torch.load(path, map_location="cpu", weights_only=False)
    if not isinstance(obj, dict):
        raise RuntimeError(f"Expected checkpoint dict, got {type(obj).__name__}")
    if obj.get("format") != "base_move_actor":
        raise RuntimeError(f"Expected format='base_move_actor', got {obj.get('format')!r}")
    actor = obj.get("actor")
...
def main() -> None:
    ...
    actor_state = load_base_move_actor(actor_path)
    out_path = out_dir / "base_move.onnx"
    obs_dim, act_dim = export_role_actor(
        role_name="base_move",
        state_dict=actor_state,
        out_path=out_path,
        opset=args.opset,
    ...
    print(f"- base_move: {out_path.name}  input=float32[batch,{obs_dim}] output=float32[batch,{act_dim}]")
